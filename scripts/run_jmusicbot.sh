...
download() {
    if [ $DOWNLOAD = true ]; then
        ...
        LATEST_RELEASE=$(curl -s https://api.github.com/repos/ragnarrok/JMusicBot/releases/latest)
        ...
        if [ "$IS_PRERELEASE" = "true" ]; then
            ...
            ALL_RELEASES=$(curl -s https://api.github.com/repos/ragnarrok/JMusicBot/releases?per_page=10)
